...
module.exports = ({ files, markdownNode, markdownAST, getNode, reporter, cache, }, pluginOptions) => {
    const defaults = {
        components: [{tagName: 'component-image', attributes: [{source: 'src', target: ''}]}],
        sharpFunction: 'fluid',
    }
    ...
    const generateImages = async (imageSource, optionsOverride) => {
        ...
        if (applyOptions.withWebp) {
            const webpResult = await sharp[applyOptions.sharpFunction]({
                file: imageNode,
                args: _.defaults({toFormat: `WEBP`}, applyOptions),
                reporter,
                cache,
            })
            if (webpResult) {
        ...
        if (applyOptions.tracedSVG) {
            let args = typeof applyOptions.tracedSVG === `object` ? applyOptions.tracedSVG : {}
    ...
    const rawHtmlNodes = []
    visitWithParents(markdownAST, 'html', (node) => { rawHtmlNodes.push(node) })
            ...
            new Promise(async (resolve) => {
                ...
                for (const componentDefinition of options.components) {
                    $(componentDefinition.tagName).each(function() {
                        processComponents.push({ ref: $(this), attributes: componentDefinition.attributes})
                    })
                }
                ...
                for (const component of processComponents) {
                    for (const attribute of component.attributes) {                    
                        ...
                        const fileType = imageSource.split(".").reverse()[0]
                        // sharp can't process gifs or svgs
